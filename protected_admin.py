...
The source credential is intentionally absent.  Desktop callers must replace a
successful credential immediately with the non-secret canonical operator id and
must use :func:`display_operator_name` for worker-facing copy.
"""
...
PROTECTED_ADMIN_PROFILE_ENV = "CONTAINER_AUDIT_PROTECTED_ADMIN_PROFILE"
...
class ProtectedAdminProfileError(RuntimeError):
    """Raised when the provisioned protected-administrator profile is unusable."""
...
def sanitize_persistent_value(value: Any, *, _field_name: str = "") -> Any:
    ...
    if isinstance(value, str):
        return value.replace(PROTECTED_ADMIN_OPERATOR_ID, PROTECTED_ADMIN_DISPLAY_NAME)
    return value
