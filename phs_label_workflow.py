...
from vendor.kmtech_zero_pe.gdi_print import (
    GdiPrinter,
    MarginsMM,
    PaperSpec,
    PrinterError,
...
from vendor.kmtech_zero_pe.raster import (
    FontSpec,
    RasterCanvas,
    RasterError,
)
...
class WindowsGDIPhysicalLabelPrinter:
    ...
    _PAPER = PaperSpec(orientation="portrait", paper_size_code=9)
    _MARGINS = MarginsMM(left=0.0, top=0.0, right=0.0, bottom=0.0)
    ...
    def print_png(
        ...
        *,
        ...
        except (PrinterError, OSError, ValueError) as exc:
            raise PHSPhysicalPrintError(
                f"실물 현품표 GDI 출력에 실패했습니다: {exc}"
            ) from exc
        return PhysicalPrintEvidence(
            printer_name=printer_name,
            spool_job_id=int(receipt.job_id),
            document_name=doc_name,
    ...
    def render(
        ...
        try:
            with RasterCanvas(1100, 600, background=(255, 255, 255)) as canvas:
                ...
                canvas.text((530, 55), "PHS 현품표", font=FontSpec(48, bold=True))
                canvas.text(
                    (530, 145),
                    f"작업일  {business_date}",
                    font=FontSpec(38, bold=True),
                )
                canvas.text(
                    (530, 215),
                    f"작업코드  {worker_code}",
                    font=FontSpec(34, bold=True),
                )
                canvas.text(
                    (530, 290),
                ...
                if item_name:
                    canvas.text((530, 345), item_name[:28], font=FontSpec(27))
                canvas.text(
                    (530, 410),
                    f"수량  {int(getattr(tray, 'tray_size', 0) or 0)} Pcs",
                    font=FontSpec(31, bold=True),
                )
                canvas.text(
                    (530, 490),
        ...
        return RenderedPHSLabel(str(output_path), str(saved["sha256"]))
