#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Generate local Phase G Container_Audit relay runtime evidence."""
...
def _make_manifest(tmp_root: Path) -> Path:
    manifest = {
        ...
        "pc_identity": {
            ...
            "source_host_id": "container-phase-g-host",
            "producer_install_id": "install-container-phase-g",
        },
        "apps": ["ContainerAudit"],
        "streams": [
            {
...
def _make_credential(tmp_root: Path) -> Path:
    ...
    _write_json(
        path,
        {
            ...
            "secret": "container-phase-g-local-secret",
...
def _write_source_file(tmp_root: Path, *, name: str = "container_phase_g.csv") -> Path:
...
def _runner_status_log_report(tmp_root: Path) -> dict:
    ...
    return {
        "status": "PASS" if ok else "FAIL",
        "scope": "local Container_Audit CSV queue/status/log runner proof with fixture session",
...
def _install_pack_dry_run_report(tmp_root: Path) -> dict:
    ...
    ok = (
        ...
        and "container-phase-g-local-secret" not in serialized
...
def _source_scan_admission_report(tmp_root: Path) -> dict:
    ...
    allowed_file = scan_dir / "이적작업이벤트로그_admission.csv"
    ignored_file = scan_dir / "unrelated.csv"
    nested_dir = scan_dir / "nested"
    nested_dir.mkdir(exist_ok=True)
    nested_allowed = nested_dir / "이적작업이벤트로그_nested.csv"
    ...
    return {
        ...
        "approved_file_family": "이적작업이벤트로그_*.csv",
...
def build_report(tmp_root: Path, report_path: Path) -> dict:
    ...
    report = {
        "report_version": "direct-sync-phase-g-container-audit-runtime-v1",
        ...
        "container_audit_runtime_relay_report": {
            ...
            **_flow_runtime_subreports(
                ...
                lost_ack_blocked_reason="No real server committed-but-local-ack-lost replay drill from a Container_Audit producer PC.",
            ),
            "local_runner_status_log_report": runner,
            "blocked_reason": "No real Container_Audit producer-PC scheduled task/service run or production direct receipts.",
        },
        ...
        "lost_ack_replay_report": {
            "status": "BLOCKED",
            "local_replay_report": lost_ack,
            "blocked_reason": "No real server committed-but-local-ack-lost replay drill from a Container_Audit producer PC.",
        },
        "reboot_recovery_report": reboot_recovery,
        ...
        "production_install_pack_report": {
            ...
            "blocked_reason": "No approved Container_Audit production install, task/service registration, smoke test, uninstall, or restore evidence.",
        },
    }
    _bind_evidence_artifact(
        report["container_audit_runtime_relay_report"],
        report_path=report_path,
        evidence_name="container_audit_runtime_relay_report",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase G local Container_Audit runtime evidence.")
    ...
    print(f"phase_g_container_audit_runtime_report={report_path}")
