...
$evidenceRoot = 'C:\Seq307\evidence'
$mappedNegative = 'C:\Seq307\mapped-negative'
$mappedPositive = 'C:\Seq307\mapped-positive'
$workRoot = 'C:\Seq307Work'
...
$expectedNegativeSha256 = '409732a65ac9a3aee5be2e5f3a0b06bd51dbcee40df5d1704844934e4b9520e8'
...
if (-not ('Seq307WindowProbe' -as [type])) {
    Add-Type -TypeDefinition @'
using System;
using System.Collections.Generic;
using System.Runtime.InteropServices;
using System.Text;
public static class Seq307WindowProbe {
...
function Get-VisibleWindowsForProcess {
    ...
    [Seq307WindowProbe]::EnumWindows({
        param($handle, $parameter)
        [uint32]$ownerProcessId = 0
        [void][Seq307WindowProbe]::GetWindowThreadProcessId($handle, [ref]$ownerProcessId)
        if ($ownerProcessId -eq $ProcessId -and [Seq307WindowProbe]::IsWindowVisible($handle)) {
            $text = [Text.StringBuilder]::new(1024)
            [void][Seq307WindowProbe]::GetWindowText($handle, $text, 1024)
            $rectangle = [Seq307WindowProbe+RECT]::new()
            [void][Seq307WindowProbe]::GetWindowRect($handle, [ref]$rectangle)
...
Write-GuestLog 'SEQ307_GUEST_START'
$startedAt = [DateTimeOffset]::UtcNow
...
try {
    ...
    # The exact current frozen Container artifact is the negative control and must run first.
    Write-GuestLog 'NEGATIVE_COPY_START'
    Copy-Item -LiteralPath $mappedNegative -Destination $negativeRoot -Recurse
    $negativeExe = Join-Path $negativeRoot 'Container_Audit.exe'
    $negativeHash = (Get-FileHash -LiteralPath $negativeExe -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($negativeHash -cne $expectedNegativeSha256) { throw 'Negative frozen Container executable identity mismatch.' }
    ...
    $negativeMatches = @(
        $negativeEvents.events | Where-Object {
            ...
            $_.id -eq 3077 -and
            $text.Contains('container_audit.exe') -and
    ...
    if (-not $negative.block_proven) {
        $verdict = 'INCONCLUSIVE_NEGATIVE_CONTAINER_NOT_BLOCKED'
        throw 'Frozen Container negative control did not produce exact VerifiedAndReputableDesktop Code Integrity 3077.'
    }
    ...
    $env:CONTAINER_AUDIT_AUTOMATED_TEST = '1'
    $env:CONTAINER_AUDIT_AUDIO_ENABLED = 'off'
    $env:KMTECH_TEST_SILENT_AUDIO = '1'
    $env:CONTAINER_AUDIT_DIRECT_SYNC_BOOTSTRAP = 'off'
    $env:CONTAINER_AUDIT_SESSION_SYNC_TRIGGER = 'off'
    $env:CONTAINER_AUDIT_UPDATE_PROVIDER = 'off'
    $env:CONTAINER_AUDIT_ENABLE_FIRST_RUN_ONBOARDING = 'off'
    $env:PYTHONDONTWRITEBYTECODE = '1'
    $env:CONTAINER_AUDIT_DATA_ROOT = Join-Path $workRoot 'data'
    $env:CONTAINER_AUDIT_PORTABLE_SMOKE_MARKER = Join-Path $evidenceRoot 'portable-smoke.json'
    $positiveBaseline = Get-LatestCiRecordId
    $launcher = Join-Path $positiveRoot 'launch-container-audit.cmd'
    ...
    $smokePass = (
        ...
        (Test-Path -LiteralPath $env:CONTAINER_AUDIT_PORTABLE_SMOKE_MARKER -PathType Leaf)
    )
    ...
    while ($null -eq $mainWindow -and [DateTimeOffset]::UtcNow -lt $windowDeadline) {
        $mainWindow = @(
            Get-VisibleWindowsForProcess -ProcessId $guiProcessId |
                Where-Object { $_.title -like '*(v2.0.99)*' } |
                Select-Object -First 1
        ) | Select-Object -First 1
    ...
    $windowAfterHold = @(
        Get-VisibleWindowsForProcess -ProcessId $guiProcessId |
            Where-Object { $_.handle -eq $mainWindow.handle -and $_.title -like '*(v2.0.99)*' } |
            Select-Object -First 1
    ) | Select-Object -First 1
    ...
    $sourceCatalogDiagnostic = Join-Path $env:LOCALAPPDATA 'KMTech\DirectSync\container_audit\status\item_catalog_startup_diagnostic.json'
    ...
    $positive = [pscustomobject][ordered]@{
        ...
        smoke_launcher = [pscustomobject][ordered]@{
            ...
            marker_exists = Test-Path -LiteralPath $env:CONTAINER_AUDIT_PORTABLE_SMOKE_MARKER -PathType Leaf
            pass = $smokePass
        }
    ...
    if ($smokePass -and $guiPass -and $positiveCiZero) {
        $verdict = 'PASS_SAC_ENFORCE_CONTAINER_NEGATIVE_THEN_PORTABLE_GUI'
    }
    else {
        $verdict = 'FAILED_SAC_ENFORCE_PORTABLE_GATE'
        throw 'Portable Container Sandbox behavior did not satisfy smoke, GUI, and zero-CI-event gates.'
    }
}
catch {
    ...
    if (
        $verdict -eq 'PASS_SAC_ENFORCE_CONTAINER_NEGATIVE_THEN_PORTABLE_GUI' -and
    ...
    $summary = [pscustomobject][ordered]@{
        schema = 'seq307-container-sandbox-sac/v1'
    ...
    Write-GuestLog 'SEQ307_EVIDENCE_COMPLETE'
