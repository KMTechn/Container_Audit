...
- 기본 profile은 `C:\ProgramData\KMTech\ContainerAudit\protected\protected_admin.json`이다. 원문 코드는 저장되지 않는다.
...
1. Container Audit를 실행할 정확한 Windows 사용자 계정을 확인한다. 예: `LINE-PC01\operator`.
...
5. `protected_admin_provision=PASS mode=installed`를 확인한 뒤 일반 작업자 계정으로 Container Audit의 관리자 인증을 현장 검증한다.
