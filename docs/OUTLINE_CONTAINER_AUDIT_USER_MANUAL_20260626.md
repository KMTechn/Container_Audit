...
> **HISTORICAL—현장 사용 금지.** 이 파일은 과거 화면 이력 보존용입니다. 작업자는 적용 commit `dc7c50dd6217e2a373d57704a6b9523862a5d7b8` 기준의 [`OUTLINE_CONTAINER_AUDIT_USER_MANUAL_20260627.md`](OUTLINE_CONTAINER_AUDIT_USER_MANUAL_20260627.md)만 따르세요.
...
> 이 파일의 2026-06-30 화면은 현재 적용 commit의 문구·배치를 증명하지 않습니다. 아래 본문과 이미지는 역사 참고이며 현장 절차의 근거로 사용하지 않습니다.
...
3. 원본 물리 PHS2를 먼저 한 번 스캔합니다.
4. 중앙 preflight와 서명 lease가 끝난 뒤 제품 바코드를 하나씩 스캔합니다.
5. 중앙 exact GOOD member의 제품 ID↔바코드 집합 전량이 일치해야 트레이가 완료됩니다.
6. 다음 현품표를 스캔합니다.

작업 데이터는 먼저 이 PC의 로컬 저장소에 기록되지만, 이것이 무조건 오프라인 진행을 뜻하지는 않습니다. PHS2 시작 전에는 중앙 exact source와 서명 lease를 받아야 하므로 preflight가 실패하면 새 트레이를 시작하지 않습니다. 정상 preflight 뒤 연결이 끊기면 현재 트레이와 실물을 그대로 보존하고 화면의 중앙 이적 결과에 따릅니다. `저장 전송`의 DirectSync 대기는 현재 트레이의 중앙 이적 승인과 별도입니다.
...
- 먼저 `중앙에서 검사 완료 수량과 제품 구성을 확인하고 있습니다.`가 표시됩니다.
- 중앙 preflight가 승인되면 가운데에 품목명, 품목코드와 중앙 exact GOOD `member_count`가 목표로 표시됩니다.
- preflight 중 들어온 입력이 `중앙 확인 중 · 보류 스캔 N건`으로 표시되면 이미 접수된 것이므로 다시 스캔하지 않습니다.
- `이번 스캔은 접수되지 않았습니다.`가 표시된 입력만 busy가 끝나고 목록·수량을 확인한 뒤 한 번 다시 스캔합니다.
...
화면 수량은 진행 확인용입니다. 중앙 exact GOOD `member_count`와 제품 ID↔바코드 집합이 모두 일치할 때만 완료되며, 같은 품목의 다른 제품으로 개수만 맞추면 완료되지 않습니다.
...
## 7. exact member 전량 완료

![목표 수량 완료 후 화면](assets/container_audit_user_manual_20260626/05-product-scan-2.png)

중앙 preflight에 고정된 exact GOOD member의 제품 ID↔바코드 집합을 전량 스캔하면 프로그램이 완료 처리를 시작합니다. 수량만 맞고 집합이 다르거나 일부만 스캔된 표준 PHS2는 완료하지 않습니다.
...
- 보류한 작업은 왼쪽 `보류 작업 N건 (더블클릭으로 복원)` 목록에 표시됩니다.

보류 작업 다시 불러오기:

1. 왼쪽의 `보류 N건 보기`를 눌러 `보류 작업 N건 (더블클릭으로 복원)` 목록을 봅니다.
2. 다시 할 트레이를 더블클릭합니다.
3. 확인창이 나오면 내용을 읽고 선택합니다.

`🔄 완료 현품표 교체`

- 표준 PHS2 exact 이적 모드에서는 기존 완료 현품표 교체 흐름을 사용하지 않습니다.
- 화면의 `중앙 교체 워크플로 필요` 또는 `관리자 교체 절차 필요` 안내를 따르고 담당자에게 알립니다.
- 이미 봉인된 이적·포장 작업을 임의로 바꾸지 않습니다.

`🔁 현재 이적 제품 교체`

- 이적 확정 전 현재 트레이에 이미 스캔된 제품을 교체할 때만 사용합니다.
- 같은 품목·입고 lot·UOM 조건을 충족하는 기존 제품↔새 양품을 1~2쌍 선택합니다.
- 중앙에서 모든 쌍이 원자적으로 확정돼야 현재 트레이에 반영됩니다. 한 쌍이라도 실패하면 현재 트레이를 유지합니다.

`트레이 제출`

- 표준 PHS2의 부분 제출에는 사용하지 않습니다.
- exact member 전량보다 적은 상태에서 누르면 `PHS=2 현품표는 일부 제출할 수 없습니다. 등록된 제품을 모두 스캔하세요.`가 표시되고 완료가 차단됩니다.
- 부족한 실물이나 집합 불일치가 있으면 트레이·스캔 목록·실물을 보존하고 담당자에게 알립니다.

## 9. 현재 이적 제품 교체

![개별 제품 교환 창](assets/container_audit_user_manual_20260626/11-product-exchange-dialog.png)

표준 제품 교체는 작업 리더 지시에 따라 이적 확정 전 현재 트레이에서만 수행합니다. 새 양품은 대상과 같은 입고 lot·품목·UOM·원장 plane의 별도 PHS에 속해야 하며, 완료된 이적·포장 작업을 사후에 바꾸는 일반 절차가 아닙니다.

진행 순서:

1. 현재 이적 트레이에 교체 대상 제품이 스캔돼 있고 아직 이적 확정 전인지 확인합니다.
2. `🔁 현재 이적 제품 교체`를 누릅니다.
3. 1~2쌍의 교체 수량을 선택합니다.
4. 현재 트레이에 있는 기존 제품을 먼저 스캔한 뒤 같은 품목 조건의 새 양품을 스캔합니다.
5. `교환 완료`를 눌러 중앙 결과를 기다립니다.
6. `중앙 교체 완료`가 확인된 경우에만 교체된 실물과 화면 목록을 다시 대조합니다.

주의:

- 기존 제품과 새 양품의 순서를 바꾸면 안 됩니다.
- 교체 수량은 1~2쌍입니다.
- 중앙 결과가 `중앙 교체 응답 대기` 또는 `중앙 교체 담당자 확인 필요`이면 현재 트레이를 유지하고 임의로 다시 스캔하지 않습니다.

## 10. 완료 현품표 교체

![완료 현품표 교체 화면](assets/container_audit_user_manual_20260626/10-master-label-replacement.png)

이 절과 이미지는 레거시 기록입니다. 표준 PHS2 exact 이적 모드에서는 기존 완료 현품표 교체가 차단되며 `중앙 교체 워크플로 필요` 또는 `관리자 교체 절차 필요`가 표시됩니다. 이미 봉인된 이적·포장 작업은 교체하지 말고 담당자에게 알립니다.

## 11. 종료와 복구

startup 복구와 보류 목록에서 다른 작업을 여는 전환은 서로 다른 창입니다. 두 창의 `아니오` 결과가 반대이므로 창 제목을 먼저 확인합니다.

| 창 제목 | `예` | `아니오` | `취소` 또는 창 닫기 |
|---|---|---|---|
| `이전 작업 복구` | 이전 트레이를 계속 작업합니다. | 이전 트레이를 **보류하고 새 작업**을 시작합니다. 삭제하지 않습니다. | 상태를 바꾸지 않고 로그인 화면으로 돌아갑니다. |
| `작업 전환 확인` | 이어지는 `트레이 보류 확인`에서 확정하면 현재 트레이를 보류한 뒤 선택한 보류 작업을 불러옵니다. | **현재 트레이를 삭제**한 뒤 선택한 보류 작업을 불러옵니다. | 현재 트레이와 보류 작업을 바꾸지 않습니다. |

`이전 작업 복구`에서 `아니오`를 선택한 트레이는 같은 작업자로 다시 로그인한 뒤 왼쪽의 `보류 N건 보기`를 열어 `보류 작업 N건 (더블클릭으로 복원)` 목록에서 찾습니다. 해당 행을 더블클릭해 복원하고 화면의 품목·`스캔 수량`을 실물과 대조한 뒤 남은 제품만 스캔합니다. 보류된 트레이와 실물은 삭제된 것이 아니므로 따로 보존합니다.
...
- 현품표 스캔 가능
- 제품 스캔 가능
- exact GOOD member 집합 전량 완료와 표준 PHS2 부분 제출 차단 확인
- `스캔 취소` 가능
- 현재 작업 리셋 가능
- 트레이 보류 및 복원 가능
- 이적 확정 전 현재 제품 1~2쌍 원자 교체 확인
- 표준 PHS2에서 레거시 완료 현품표 교체 차단 확인
- 종료 후 복구 가능
- 로컬 events 폴더에 CSV 생성
- `저장 전송` 카드의 `정상 대기` 또는 `전송 정상`과 현재 트레이 중앙 이적 결과를 따로 확인
- 서버 당일 조회에 완료 기록 반영
...
> 코디네이터 직접 확인(D-121, 2026-09-03) 결과 아래 세 원본 경로는 부재하지만, 회사 프로그램 아카이브에는 원래 캡처 세트 10개(9 PNG + 1 JSON)가 남아 있습니다. 따라서 과거 보존 evidence는 `부분 존재(아카이브)`로 분류하고 historical 참고로만 취급합니다.
...
- 확인된 아카이브: `E:\KMTech\company-program-archive-20260821-postreboot\moved-unique\.deploy_backups\Container_Audit_local_state_20260625-124732\.codex\uiux-captures\20260623-213546-full-uiux-background`

정본은 `E:/KMTech/production-readiness-20260830/HANDOVER/CAPTURE-BUNDLE-V1-CONTRACT.md`입니다. 현행 승인 묶음은 앱 저장소 밖의 `D:/KMTech/cold-program-material/from-E/requal-evidence/capture-bundle-v1/<app>/<app>__<commit12>__<YYYYMMDDTHHMMSSZ>__<nonce8>/`에 둡니다. `HANDOVER-INDEX.md`가 가리키는 불변 `indexes/handover-index__<YYYYMMDDTHHMMSSZ>__<nonce8>.json`에서 `app=Container_Audit`을 찾고 bundle `manifest.json`의 `captures[].state_id`로 조회하며, 저장소에서는 GUI 없이 `python -B tools/capture_container_operator_ui.py --describe-m7-contract`의 `{schema, app, required_state_ids}` envelope로 선언을 확인합니다.

필수 state ID는 `m7_phs2_preflight`, `m7_central_preflight_queue`, `m7_completion_busy`, `m7_recovery_transition`, `m7_direct_sync_backlog_ack`, `m7_exact_good_membership`, `m7_lease_fail_closed`, `m7_transfer_receipt_status`, `m7_partial_atomic_exchange`의 아홉 개입니다. 디렉터리·파일명, manifest field와 create-new 순서는 위 정본만 따르며 이 문서에는 그 계약을 재정의하거나 미래 digest를 기록하지 않습니다. 승인자와 evidence owner는 `미정 — 조직 확정 필요(Q1)`이고, C-1은 `external bundle 캡처 대기(재감사 통과 전 '도구 준비됨' 표기 금지)`, C-4는 `도구 정정 진행(재감사 대기)`, C-5는 `부분 존재(아카이브)`입니다.
