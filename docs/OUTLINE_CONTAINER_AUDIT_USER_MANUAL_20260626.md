...
작업 데이터는 먼저 이 PC의 로컬 저장소에 저장됩니다. 그래서 인터넷이 잠시 끊겨도 당일 작업은 계속할 수 있습니다. 서버 연결이 가능해지면 Direct Sync가 서버로 업로드합니다. 배포 버전은 Syncthing 폴더가 없어도 동작하는 것을 기준으로 합니다.
