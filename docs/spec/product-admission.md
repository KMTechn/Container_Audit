...
결과의 status/event/detail과 일반·held 호출자의 사건·저장·FIFO ACK 순서는 유지한다.
형식 오류 문구는 아래처럼 상황과 유효한 다음 행동을 함께 안내한다.
동일 품목 통과가 중앙 GOOD membership을 증명하지 않으며,
...
새 구현에 맞춰 기대 로직을 바꾸지 않는다. U06에서 바뀐 안내만 기존 상황 설명과
다음 행동의 의미를 검사하고, 결정 전체·입력 무변경 비교는 유지한다.
실제 새 정책·공용 leaf·schema는 후속 요구다.

## 형식 오류 안내의 다음 행동

| 사유 | 상황·다음 행동 |
|---|---|
| 너무 짧음 | 품목코드보다 길어야 함을 안내하고 제품 라벨 확인 후 다시 스캔; 같은 오류가 계속되면 담당자에게 라벨 형식 확인 |
| 128자 초과 | 길이 제한을 안내하고 제품 라벨 형식을 담당자에게 확인 |
| 외곽 공백 / 제어 문자 | 각각의 원인을 안내하고 스캐너 설정 확인 후 다시 스캔 |
| 수식 시작 / HTML·script / 경로 형식 | 각각의 금지 형식을 안내하고 제품 라벨과 스캐너 설정을 담당자에게 확인 |
| 품목코드 길이 설정 / 스캔 기록 / 목표 수량 오류 | 해당 설정·상태를 관리자에게 문의; 반복 스캔 지시 없음 |
| 품목 정보 없음 | 현재 트레이의 현품표 확인 |
| 유효한 길이가 없는 짧음 진단 | 일반 형식 오류와 제품 라벨 확인 후 다시 스캔; 같은 오류가 계속되면 담당자에게 라벨 형식 확인 |
| 미지 사유 | 일반 형식 오류와 제품 라벨 확인 |

정상·품목 불일치·중복·초과는 기존 호출자 표시를 유지한다. 제품 바코드와 현품표를
구별하며, 거부를 저장/완료로 표시하지 않는다. 안내에 위험 원문이나 영문 reason을
추가하지 않고 기존 event reason·SHA256·길이 진단 및 형식 오류의 수량 증가 0을 유지한다.
GUI의 기존 외곽 공백 strip과 포트 직접 호출의 공백 거부는 별도이며 새 정규화는 없다.
