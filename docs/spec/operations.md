...
작은 미커밋 수정 후보는 공통 fixture의 7개 원본 바이트를 `copyfile`로 임시 packet에 복사한다. 원본 readonly를 풀거나 source hash 검사를 완화하지 않는다. 별도 회귀의 host RED1→GREEN1과 실제 두 영향 사례2 PASS/31.64초, 기록된 relay6개 부재를 [근거](E:/KMTech/ca-execution-rp-0908/CORRECTED-TWO-CHILDREN.json)에 남겼다. 별도 E 제어 후보는 이미 보유한 native handle의 `QueryFullProcessImageName` 결과를 사용하며 PS5/PS7에서 실제 image·자연 종료·양쪽 EOF 및 invalid handle 거부를 확인했다. 기존 제어/reader와 FULL 조건은 변경하지 않았다. Main의 독립 검토·새 packet 결속 전에는 변경된 guest 실행을 하지 않으며, 별도 FULL·build·설치·재시작·재설치·rollback·backend E2E는 **NOT TESTED**, Ready0/6이다.
