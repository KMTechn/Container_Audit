...
- 일반 이벤트 append 실패는 200ms UI 상태 확인을 통해 저장 실패·재시도 안내를 표시하고, writer는 대기/재시작 때 `events/_event_outbox`의 원 payload를 재생한다. 보존 사본까지 쓰지 못한 경우 접수는 False이며 메모리에 원 payload를 유지하고 저장 공간 확인을 안내한다. 이 상태에서는 정상 종료를 거부한다. 사본이 내구 저장된 append 실패는 재시작 복구 가능하며, 전원 차단 시 메모리만 남은 사건까지 보존한다고 주장하지 않는다.
