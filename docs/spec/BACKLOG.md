...
| 실제 연동·수용 | CA238 소스 통합 시험 및 현재 유한 비교 수용, 목표 설치 조합은 미입증 | 238 PASS/714 ordered phase PASS와 원래 보존/reader FAILED를 분리. 이번 새 실행은 NOT TESTED. [CA-G07](#ca-g07). |
...
- **담당·의존·다음 행동:** CA 담당+Main, 중앙 [Q01/Q07](../../../Program_Spec_Hub/BACKLOG.md#qualification). Main이 준비된 정확한 14개 경로를 검토·커밋한 뒤 기존 FULL admission과 exact build/freeze, 설치 업무·중앙 도착·restart/cold boot·정상 제거·동일 packet 재설치·rollback·실제 통합 E2E로 연결한다. 해당 후속 범위는 **NOT TESTED**, Ready는 0/6이다. [기존 실행/빌드 경로와 남은 기준](E:/KMTech/coordinator-handoff-20260907-01a07992/ca-validator-source-unit-close/worker/NEXT-ROUTE.md)은 계획만 기록한다. 알려진 index metadata 동작을 반복하려고 변경 없는 238개 시험을 재실행할 필요는 없으며 실패 원본/동결 VM source/기존 근거는 불변으로 남긴다.
