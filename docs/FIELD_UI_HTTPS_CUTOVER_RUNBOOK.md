...
- 이 canary는 Syncthing 제거 승인으로 간주하지 않는다. 20PC 동시성, Syncthing shadow no-double-count, 실제 스캐너 UI 전체 시나리오, rollback rehearsal, downstream 화면 검증은 별도 승인된 window에서 계속 필요하다.
