...
- 배포용 enrollment token은 설치 패키지, ZIP, Git, 증적 bundle에 포함하지 않는다. 필요한 경우 짧은 수명의 범위 제한 token을 ACL 보호된 env/file로만 전달하거나, 승인된 작업자 PC/VPN IP allowlist 방식으로 self-enroll한다. 각 PC는 `register_container_audit_worker_pc.py --self-enroll`로 PC별 identity와 HMAC key를 자동 등록하되, token 값은 리포트/로그/스크린샷에 남기지 않는다.
...
- 2026-06-26 01:54 KST 임시 IP 탐색 창: 운영 서버 self-enroll allowlist를 `0.0.0.0/0,::/0`로 열어 IP를 수집한 방식은 **retired/forbidden**이다. 이 기록은 과거 증적일 뿐 재사용 가능한 운영 패턴이 아니다. 이후 self-enroll allowlist는 사전 승인된 작업자 PC/VPN CIDR만 허용해야 하며, 임시 CIDR은 승인된 짧은 window 뒤 access log와 설정 diff로 closure proof를 남기고 즉시 닫는다. 이 PC의 당시 캡처 IP는 nginx log 기준 `cf=114.204.147.29`, `xff=114.204.147.29`였고 증거 파일은 `C:\company\program\Container_Audit\.tmp\self-enroll-open-ip-capture-20260626\worker_pc_registration_no_token_open_allowlist.json`이다.
