...
_TEST1_APP_DATA_ROOT_ENV = "CONTAINER_AUDIT_DATA_ROOT"
_TEST1_LEGACY_BASE_URL_ENV = "WORKER_ANALYSIS_LOGISTICS_API_BASE_URL"
_TEST1_LEGACY_TOKEN_ENV = "WORKER_ANALYSIS_LOGISTICS_API_TOKEN"
_TEST1_LEGACY_SCOPE_ENV = "WORKER_ANALYSIS_LOGISTICS_AUTHORITY_SCOPE_ID"
_TEST1_LEGACY_SOURCE_ENV = "WORKER_ANALYSIS_LOGISTICS_SOURCE_HOST_ID"
_TEST1_LEGACY_DEVICE_ENV = "WORKER_ANALYSIS_LOGISTICS_DEVICE_ID"
...
def unprotect_machine_secret(value: bytes) -> str:
    ...
    try:
        ...
        raise LogisticsRuntimeConfigurationError(
            "DPAPI bearer token is not valid UTF-8"
        ) from exc
...
def _test1_isolated_legacy_override_enabled() -> bool:
    ...
    _app_data_root, run_name = _test1_run_path(
        ...
        label="TEST1 isolated Container data root",
    )
...
def _read_profile(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise LogisticsRuntimeConfigurationError(
            "machine logistics profile could not be read"
        ) from exc
...
def _resolve_secret_path(profile_path: Path, reference: Any) -> Path:
    ...
    try:
        ...
        raise LogisticsRuntimeConfigurationError(
            "bearer_token_ref escapes the profile directory"
        ) from exc
    return resolved
...
def load_logistics_runtime_profile(
    ...
    *,
    ...
    return LogisticsRuntimeProfile(
        ...
        tls_ca_bundle_path=(
            isolated_context.ca_bundle_path
            if isolated_context
            else _resolve_tls_ca_bundle_path(
