...
def _validate_remote_host_url(parsed: Any, *, label: str) -> None:
    ...
    try:
        ...
        raise DirectSyncPushError(f"{label} port is invalid") from exc
    host = parsed.hostname.strip().lower()
    if host == "localhost" or host.endswith(".localhost"):
...
def restore_raw_artifact_to_file(
    *,
    ...
    if not 200 <= status_code < 300:
        payload, _valid = _response_json(response)
    ...
    try:
        _write_json_atomic(
            status_path,
            {
                ...
                "status_context": redact_runtime_secrets(context),
    ...
    *,
    ...
    return UploadResult(
        ...
        _runtime_token_unconsumed=(
            not committed
            and payload_is_valid_json
            and committed_value is False
            and status_code >= 400
        ),
    )
